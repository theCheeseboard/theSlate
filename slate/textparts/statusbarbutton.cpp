...
void StatusBarButton::paintEvent(QPaintEvent *event) {
    ...
    QRect textRect;
    textRect.setWidth(this->fontMetrics().horizontalAdvance(this->text()) + 1);
...
QSize StatusBarButton::sizeHint() const {
    return QSize(this->fontMetrics().horizontalAdvance(this->text()) + 1 + SC_DPI(6), this->fontMetrics().height() + SC_DPI(6));
}
